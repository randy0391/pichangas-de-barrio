...
export const RegisterPage = () => {
    ...
    const [formData, setFormData] = useState({
        name: '', email: '', password: '', password_confirmation: '', phone: '', position: 'medio'
    });
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-md mx-auto mt-10 p-6 bg-white rounded shadow">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Input type="password" placeholder="Confirmar Contraseña" value={formData.password_confirmation} onChange={e => setFormData({...formData, password_confirmation: e.target.value})} required />
                <Input placeholder="Teléfono (Opcional)" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
                <select className="w-full p-2 border rounded" value={formData.position} onChange={e => setFormData({...formData, position: e.target.value as any})}>
                    <option value="portero">Portero</option>
                    <option value="defensa">Defensa</option>
                    <option value="medio">Medio</option>
                    <option value="delantero">Delantero</option>
                </select>
                <Button type="submit" disabled={isLoading} className="w-full">Registrarse</Button>
            </form>
            <div className="mt-4 text-center">
                <Link to="/login" className="text-blue-500">¿Ya tienes cuenta? Inicia sesión</Link>
            </div>
        </motion.div>
    );
};
...
export const NewsPage = () => {
    ...
    const tabs = ['Todas', 'Noticias', 'Anuncios', 'Novedades'];
...
write('src/features/news/NewsDetailPage.tsx', `
...
export const NewsDetailPage = () => {
    ...
    return (
        <div className="container mx-auto p-4 max-w-3xl">
            ...
            {post.featured_image && <img src={post.featured_image} alt={post.title} className="w-full h-64 object-cover rounded mb-6" />}
            <h1 className="text-4xl font-bold mb-4">{post.title}</h1>
            <div className="flex gap-4 text-gray-500 mb-8 border-b pb-4">
                <span>{post.author?.name || 'Admin'}</span>
                <span>{format(new Date(post.created_at || Date.now()), 'PPP', { locale: es })}</span>
            </div>
...
export const EventsPage = () => {
    ...
    const [filter, setFilter] = useState<'Proximos' | 'Completados'>('Proximos');

    const filtered = events.filter(e => filter === 'Proximos' ? e.status === 'upcoming' : e.status === 'completed');
    ...
    return (
        <div className="container mx-auto p-4">
            ...
            <div className="flex gap-2 mb-6">
                <button onClick={() => setFilter('Proximos')} className={\`px-4 py-2 rounded \${filter === 'Proximos' ? 'bg-green-800 text-white' : 'bg-gray-200'}\`}>Próximos</button>
                <button onClick={() => setFilter('Completados')} className={\`px-4 py-2 rounded \${filter === 'Completados' ? 'bg-green-800 text-white' : 'bg-gray-200'}\`}>Completados</button>
            </div>
                    ...
                    <Card key={event.id}>
                        ...
                        <CardContent>
                            ...
                            <div className="text-sm font-semibold mb-4">
                                {format(new Date(event.date + 'T' + event.time), 'PPP p', { locale: es })}
                            </div>
                            {user && filter === 'Proximos' && (
    ...
    return (
        <div className="container mx-auto p-4 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card><CardHeader><CardTitle>Convocatorias Próximas</CardTitle></CardHeader><CardContent className="text-3xl font-bold">{dashboard?.stats.upcoming_convocatorias}</CardContent></Card>
                <Card><CardHeader><CardTitle>Eventos Registrados</CardTitle></CardHeader><CardContent className="text-3xl font-bold">{dashboard?.stats.registered_events}</CardContent></Card>
                <Card><CardHeader><CardTitle>Total Participaciones</CardTitle></CardHeader><CardContent className="text-3xl font-bold">{dashboard?.stats.total_participations}</CardContent></Card>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                    ...
                    <div className="space-y-4">
                        {dashboard?.next_convocatorias.map((c: any) => (
                            <Card key={c.id}>
...
export const ConvocatoriaDetailPage = () => {
    ...
    const confirmados = c.participants?.filter(p => p.status === 'confirmed') || [];
    const rechazados = c.participants?.filter(p => p.status === 'rejected') || [];
    const pendientes = c.participants?.filter(p => p.status === 'pending') || [];
...
write('src/features/profile/ProfilePage.tsx', `
    ...
    return (
        <div className="container mx-auto p-4 max-w-xl">
            ...
            <div className="mb-6 flex justify-center">
                <Avatar className="w-24 h-24"><AvatarImage src={user?.avatar}/><AvatarFallback>{user?.name?.[0]}</AvatarFallback></Avatar>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Input placeholder="Dorsal" value={formData.jersey_number} onChange={e => setFormData({...formData, jersey_number: e.target.value})} />
                <Textarea placeholder="Biografía" value={formData.bio} onChange={e => setFormData({...formData, bio: e.target.value})} />
    ...
    return (
        <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                <Card><CardHeader><CardTitle>Total Miembros</CardTitle></CardHeader><CardContent className="text-3xl font-bold">{stats?.total_users}</CardContent></Card>
            ...
            {/* Add recent lists here if data available */}
        </div>
    );
};
`);
                ...
                <tbody>
                    {events.map(e => (
                        <tr key={e.id}>
                            ...
                            <td className="p-4 border-b">{e.date} {e.time}</td>
...
export const AdminConvocatoriasPage = () => {
    const { data: convs = [], isLoading } = useConvocatorias();
